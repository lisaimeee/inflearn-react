...
function isFull(tileList) {
  const emptyTileList = _.filter(tileList, (tile) => tile.value !== 0);
  const containsMerged = _.filter(tileList, (tile) => tile.isMerged).length > 0;
  return emptyTileList.length === 16 && !containsMerged;
}
...
export function makeTile(tileList) {
  ...
  while (
    !isFull(tileList) &&
    (!tile || (tileList && checkCollision(tileList, tile)))
  ) {
    console.log('jhi');
    tile = {
      id: currentId++,
